...
DESCRIPTION = "This module provides stretchy IK on the joint chain, with the ability to lock the stretchiness with a 0->1 slider. Note: IK handle is not space-switchable, and for that reason has frozen transformations at it's creation position."
...
class BasicStretchyIK(controlModule.ControlModule):
    ...
    def install_custom(self, joints, moduleGrp, moduleContainer):
        ...
        handleControlInfo = controlObjectInstance.create(name, "cubeLocator.ma", self, lod=1, translation=True, rotation=False, globalScale=False, spaceSwitching=False)
    ...
    def UI(self, parentLayout):
        ...
        # Build IK settings inside the control's collapsible.
        def _add_ik_settings(layout):
            settingsGroup = QtWidgets.QGroupBox("IK Settings")
            settingsLayout = QtWidgets.QVBoxLayout(settingsGroup)
            settingsLayout.setContentsMargins(4, 4, 4, 4)

            # Twist: -360 to 360.
            try:
                twistVal = cmds.getAttr(ikHandleControl + ".twist")
            except Exception:
                twistVal = 0.0
            twistWidget = animation_UI.FloatAttrControlWidget(
                ikHandleControl + ".twist",
                "Twist:",
                -360.0,
                360.0,
                twistVal,
                ...
                decimals=0,
                singleStep=1.0,
                callback=lambda v: cmds.setAttr(ikHandleControl + ".twist", v),
            )
            settingsLayout.addWidget(twistWidget)

            # Stretchiness: 0 to 1.
            try:
                stretchVal = cmds.getAttr(ikHandleControl + ".stretchiness")
            except Exception:
                stretchVal = 1.0
            stretchWidget = animation_UI.FloatAttrControlWidget(
                ikHandleControl + ".stretchiness",
                "Stretchiness:",
                0.0,
                1.0,
                stretchVal,
                ...
                decimals=2,
                singleStep=0.01,
                callback=lambda v: cmds.setAttr(ikHandleControl + ".stretchiness", v),
            )
            settingsLayout.addWidget(stretchWidget)

            layout.addWidget(settingsGroup)

        controlObjectInstance.UI(parentLayout, extraContentBuilder=_add_ik_settings)
    ...
    #     endPos = cmds.xform(blueprintJoints[len(blueprintJoints)-1], q=True, worldSpace=True, translation=True)
