...
class LegIK_ReverseFoot(circleIK.CircleControlStretchyIK):
    ...
    def UI(self, parentLayout):
        ...
        def _add_stretch(layout):
            # Group box for stretch controls
            stretchGroup = QtWidgets.QGroupBox("Stretch")
            stretchLayout = QtWidgets.QVBoxLayout(stretchGroup)
            try:
                val = cmds.getAttr(footControl + ".stretchiness")
            except Exception:
                val = 1.0
            stretchWidget = animation_UI.FloatAttrControlWidget(
                footControl + ".stretchiness",
                "",  # hide label
                0.0,
                1.0,
                val,
                ...
                decimals=2,
                singleStep=0.01,
                callback=lambda v: cmds.setAttr(footControl + ".stretchiness", v),
            )
            stretchWidget.label.hide()
            stretchWidget.label.setFixedWidth(0)
            stretchLayout.addWidget(stretchWidget)
            layout.addWidget(stretchGroup)

        controlObjectInstance.UI(parentLayout, extraContentBuilder=_add_stretch)
