...
DESCRIPTION = "This module provides stretchy IK on the joint chain, with the ability to lock the stretchiness. IK twist is controlled by an adjustable circle rotation control."
...
class CircleControlStretchyIK(controlModule.ControlModule):
    ...
    def install_custom(self, joints, moduleGrp, moduleContainer, createHandleControl=True, poleVectorAtRoot=True):
        ...
        creationPoseJoint = utils.findJointChain(creationPoseRoot)

        targetJoints = [creationPoseJoint[0], creationPoseJoint[1], creationPoseJoint[2]]
    ...
    def UI(self, parentLayout):
        ...
        if cmds.objExists(ikHandleControl):
            ...
            def _add_stretchiness(layout):
                try:
                    val = cmds.getAttr(ikHandleControl + ".stretchiness")
                except Exception:
                    val = 1.0
                ...
                layout.addWidget(stretch_widget)

            controlObjectInstance.UI(parentLayout, extraContentBuilder=_add_stretchiness)
    ...
    def UI_preferences(self, parentLayout):
        ...
        if not cmds.objExists(twistOffset):
            return

        try:
            current = cmds.getAttr(twistOffset + ".translateX")
        except Exception:
            current = 0.0

        widget = animation_UI.FloatAttrControlWidget(
            twistOffset + ".translateX",
            "Twist Offset:",
            -100,
            100,
            current,
            conversion=1,     # 1:1 mapping so slider matches integer values
            decimals=0,       # display as integers
            singleStep=1.0,   # step by whole units
            callback=lambda v: cmds.setAttr(twistOffset + ".translateX", v),
        )
